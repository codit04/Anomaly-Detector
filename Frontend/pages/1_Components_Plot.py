...
def plot_components(df):
    ...
    fig = make_subplots(
        ...
        cols=1,
        shared_xaxes=True,
        subplot_titles=("Trend", "Seasonal", "Residual", "Observed"),
    )
    fig.add_trace(
        go.Scatter(x=df.ds, y=result.trend, mode="lines", name="Trend"), row=1, col=1
    )
    fig.add_trace(
        go.Scatter(x=df.ds, y=result.seasonal, mode="lines", name="Seasonal"),
        row=2,
        col=1,
    )
    fig.add_trace(
        go.Scatter(x=df.ds, y=result.resid, mode="lines", name="Residual"), row=3, col=1
    )
    fig.add_trace(
        go.Scatter(x=df.ds, y=result.observed, mode="lines", name="Observed"),
        row=4,
        col=1,
    )
